defmodule Backgammon.Rules.Trictrac.Classique.State do
  ...
  def trous_for(trictrac, :white), do: get_in(ensure(trictrac), [:score, Access.at(0), :trous]) || 0
  def trous_for(trictrac, :black), do: get_in(ensure(trictrac), [:score, Access.at(1), :trous]) || 0
  def points_for(trictrac, :white), do: get_in(ensure(trictrac), [:score, Access.at(0), :points]) || 0
  def points_for(trictrac, :black), do: get_in(ensure(trictrac), [:score, Access.at(1), :points]) || 0
  ...
  def remove_all_used(values, used), do: Enum.reduce(used, values, fn die, acc -> remove_first(acc, die) end)
  ...
  def normalize_opening(opening) do
    ...
    %OpeningState{
      ...
      jan_rencontre_checked: fetch(opening, :jan_rencontre_checked, default.jan_rencontre_checked),
      coups_by_type: normalize_color_map(fetch(opening, :coups_by_type, default.coups_by_type), 0),
  ...
  def normalize_obligation(obligation) do
    %Obligation{
      ...
      must_conserve:
        normalize_conservation_candidates(fetch(obligation, :must_conserve, []), requirement?: true)
    }
  end
  ...
  defp fetch(map, key, default) when is_map(map), do: Map.get(map, key, Map.get(map, stringify_key(key), default))
