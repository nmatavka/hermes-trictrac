defmodule Backgammon.Rules.Trictrac.Classique.Scoring do
  alias Backgammon.Rules.Trictrac.Classique.{Constants, ScoreEvent, State}

  def apply_points(trictrac, color, points, label, turn_number, source \\ nil, metadata \\ %{}) do
    ...
    trous_gain = if base_trous > 0, do: base_trous * if(score.doubling_active, do: 2, else: 1), else: 0

    updated_score =
      score
      |> Map.put(:points, rem(total, 12))
      |> Map.put(:trous, trous_before + trous_gain)
      |> Map.put(:doubling_active, true)
      |> Map.put(:bredouille, true)
  ...
  defp apply_etendard([white, black]) do
    cond do
      white.trous > 0 and black.trous == 0 ->
        [%{white | etendard: true, grande_bredouille: true}, %{black | etendard: false, grande_bredouille: false}]

      black.trous > 0 and white.trous == 0 ->
        [%{white | etendard: false, grande_bredouille: false}, %{black | etendard: true, grande_bredouille: true}]

      true ->
        [%{white | etendard: false, grande_bredouille: false}, %{black | etendard: false, grande_bredouille: false}]
    end
  end
end
