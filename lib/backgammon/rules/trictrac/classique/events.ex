defmodule Backgammon.Rules.Trictrac.Classique.Events do
  ...
  def detect_turn_events(start_board, end_board, variant, color, dice, trictrac) do
    ...
    context = %{
      ...
      pile_misere: pile_misere_resolution(trictrac, end_board, color, branches_info, is_double)
    }
  ...
  defp detect_classique_turn_events(context) do
    depart_done = get_in(context.opening, [:depart_done_by_type, context.color]) || %{two_tables: false, meseas: false, six_tables: false}
  ...
  defp eval_jan_rencontre({events, context}) do
    {events, opening} = Opening.detect_jan_rencontre(events, context.color, context.dice, context.opening)
    ...
    events =
      Enum.reduce(0..23, events, fn pos, acc ->
        if Moves.pieces_at(context.start_board, pos, opp) == 1 do
          ...
          base = table_base_points(pos, opp, context.is_double)

          acc =
            if ways.true_ways > 0 do
              acc ++
                [
                  Scoring.event(context.color, "jan de recompense", base * ways.true_ways, %{
                    target: pos,
                    mode: :a_vrai,
          ...
          false_ways = if ways.true_ways > 0, do: 0, else: ways.false_ways

          if false_ways > 0 do
            acc ++
              [
                Scoring.event(opp, "jan qui ne peut", base * false_ways, %{
                  target: pos,
                  mode: :a_faux,
  ...
  defp eval_coin_battu({events, context}) do
    ...
    events =
      if length(context.dice.values || []) >= 2 and
        ...
        base = if(context.is_double, do: 6, else: 4)
        true_ways = coin_battu_true_ways(context.end_board, context.color, context.dice)
        false_ways = if true_ways > 0, do: 0, else: coin_battu_false_ways(context.end_board, context.color, context.dice)
    ...
    events =
      Enum.reduce(Constants.scoring_tables_for_variant(context.variant), events, fn table, acc ->
        ...
        if !start_full and end_full do
          ...
          ways = remplissage_way_count(context.start_board, context.color, table, context.dice, context.branches_info)
          points = (if(context.is_double, do: 6, else: 4)) * ways
          ...
          table &&
            (Moves.all_paired?(context.end_board, context.color, table.from, table.to) or
               (candidate.allow_sortie and
                  (context.end_board.outside[context.color] || 0) > (candidate.outside_before || 0)))
  ...
  defp eval_pile_misere({events, context}) do
    events =
      if Moves.pieces_at(context.end_board, State.own_coin(context.color), context.color) >= 15 do
        ...
        events ++ [Scoring.event(context.color, "pile de misere", candidate.points, %{mode: candidate.mode, resolution: :earned_now})]
      else
        events
      end
      ...
      if context.trictrac.options["margotEnabled"] and length(context.dice.values || []) >= 2 do
        ...
        distances =
          if context.is_double do
            ...
            [hd(context.dice.values), List.last(context.dice.values), Enum.sum(context.dice.values)]
          end
        ...
        if ways > 0 do
          events ++ [Scoring.event(opp, "Margot la fendue", (if(context.is_double, do: 4, else: 2)) * ways)]
        else
          events
        end
      else
  ...
  defp eval_impuissance({events, context}) do
    ...
    grand_like? =
      Enum.any?(context.conservation_candidates, fn candidate ->
        candidate.key in [:grand, :retour]
      end)

    events =
      if points > 0 and grand_like? do
        events ++
          [
  ...
  defp eval_sortie({events, context}) do
    ...
    events =
      if outside_after >= context.variant.total_pieces and outside_after > outside_before do
        events ++ [Scoring.event(context.color, "sortie", if(context.is_double, do: 6, else: 4))]
      else
        events
      end
  ...
  def pile_misere_resolution(trictrac, end_board, color, branches_info, is_double) do
    points = if(is_double, do: 6, else: 4)
    already_pending = get_in(trictrac, [:pile_misere_pending_by_type, color]) || false
    still_piled = Moves.pieces_at(end_board, State.own_coin(color), color) >= 15 and pile_misere_countable?(branches_info, color)
  ...
  defp jan_missing_info(board, color, from_norm, to_norm) do
    Enum.reduce(from_norm..to_norm, %{missing_units: 0, single_pos: [], empty_pos: []}, fn norm, acc ->
    ...
    if State.double?(dice) do
      ...
      true_ways =
        (if has_source_at_distance?(board, color, target, a, disallow_coin_single), do: 1, else: 0) +
          (if has_source_at_distance?(board, color, target, b, disallow_coin_single), do: 1, else: 0) +
          if(
  ...
  defp table_base_points(target, opp, is_double) do
    opp_norm = State.norm_pos(target, opp)
    petit_like = opp_norm >= 18 or opp_norm < 6

    cond do
      petit_like and is_double -> 6
      petit_like -> 4
      is_double -> 4
      true -> 2
    end
  ...
  defp maybe_add_coin_battu_a_faux(events, opp, target, base, ways) do
    events ++
      [
        Scoring.event(opp, "coin battu a faux", base * ways, %{
          target: target,
          mode: :a_faux,
          resolution: :opponent_beneficiary,
          false_ways: ways
