defmodule Backgammon.Rules.Trictrac.Combine.CurrentPartie do
  use Backgammon.Rules.Trictrac.AccessStruct,
    fields: [
      trous: %{white: 0, black: 0},
      legal_stop_by: %{white: false, black: false},
      uninterrupted_by: %{white: false, black: false},
      pending_result: nil
    ]
end

defmodule Backgammon.Rules.Trictrac.Combine.Track do
  alias Backgammon.Rules.Trictrac.Combine.CurrentPartie
  ...
  use Backgammon.Rules.Trictrac.AccessStruct,
    fields: [
      ...
      continuation_active: %{white: false, black: false},
      continuation_resolved_at: %{white: 0, black: 0},
      partie_over: false,
      winner: nil,
...
defmodule Backgammon.Rules.Trictrac.Combine.SuspensionState do
  use Backgammon.Rules.Trictrac.AccessStruct,
    fields: [
      suspended_track: nil,
      frozen_by: nil,
...
defmodule Backgammon.Rules.Trictrac.Combine do
  alias Backgammon.Rules.Trictrac.Combine.{CurrentPartie, SuspensionState, Track}
  ...
  def record_turn(trictrac, color, deltas, _points_gained, opts \\ []) do
    ...
    candidate =
      cond do
        own_delta > 0 and current_partie.trous[color] >= 12 -> {color, :own}
        opp_delta > 0 and current_partie.trous[opp] >= 12 -> {opp, :opponent}
        true -> nil
      end

    cond do
      current_partie.pending_result == nil and candidate != nil ->
        {winner, reached_by} = candidate

        trictrac
        |> put_in(
          [:track_classique_honneurs, :current_partie, :pending_result],
          %{
            winner: winner,
        ...
        |> put_in([:track_classique_honneurs, :current_partie, :legal_stop_by, :white], false)
        |> put_in([:track_classique_honneurs, :current_partie, :legal_stop_by, :black], false)
        |> put_in(
          [:track_classique_honneurs, :current_partie, :legal_stop_by, winner],
          reached_by == :own
        )

      not is_nil(current_partie.pending_result) and current_partie.pending_result.winner == color and own_delta > 0 ->
        put_in(
          trictrac,
          [:track_classique_honneurs, :current_partie, :legal_stop_by, color],
          true
        )
  ...
  def continuation_due?(trictrac, color) do
    current_partie = get_in(ensure(trictrac), [:track_classique_honneurs, :current_partie]) || %{}
    pending_result = current_partie.pending_result || %{}

    pending_result[:winner] == color and (current_partie.legal_stop_by || %{})[color]
  end

  def suspension_choices(trictrac, color, a_ecrire_due?) do
    classique_due? = continuation_due?(trictrac, color)
  ...
  def apply_continuation(trictrac, color, decision) do
    trictrac = ensure(trictrac)

    case decision do
      "continuer" ->
        trictrac
        |> put_in([:track_classique_honneurs, :continuation_active, color], true)
        |> put_in([:track_classique_honneurs, :continuation_resolved_at, color], current_partie_trous(trictrac, color))
        |> put_in([:track_classique_honneurs, :current_partie, :legal_stop_by, color], false)

      "arreter" ->
        trictrac
        |> put_in([:track_classique_honneurs, :continuation_active, color], false)
        |> put_in([:track_classique_honneurs, :continuation_resolved_at, color], current_partie_trous(trictrac, color))
        |> settle_current_partie(color)
  ...
  def maybe_pick_classique_winner(trictrac) do
    trictrac = ensure(trictrac)
    pending_result = get_in(trictrac, [:track_classique_honneurs, :current_partie, :pending_result])

    winner =
      if pending_result do
        pending_result.winner
      end
    ...
    if is_nil(winner) do
      ...
      trictrac
      |> put_in([:track_classique_honneurs, :partie_over], true)
      |> put_in([:track_classique_honneurs, :winner], winner)
    end
  end

  defp settle_current_partie(trictrac, color) do
    trictrac = ensure(trictrac)
    current_partie = trictrac.track_classique_honneurs.current_partie
    pending_result = current_partie.pending_result

    if pending_result && pending_result.winner == color do
      class = pending_result.class
      value = honneur_value(class)
      opp = opposite(color)
      carry = max(current_partie.trous[color] - 12, 0)

      trictrac
      |> update_in([:track_classique_honneurs, :honneurs, color], &(&1 + value))
      |> update_in([:track_classique_honneurs, :classes, color, class], &(&1 + 1))
      |> put_in([:track_classique_honneurs, :partie_over], true)
      |> put_in([:track_classique_honneurs, :winner], color)
      |> put_in(
        [:track_classique_honneurs, :current_partie],
        %{
          trous: %{white: if(color == :white, do: carry, else: 0), black: if(color == :black, do: carry, else: 0)},
          legal_stop_by: %{white: false, black: false},
          uninterrupted_by: %{white: false, black: false},
          pending_result: nil
        }
      ...
      |> put_in([:track_classique_honneurs, :continuation_active, color], false)
      |> put_in([:track_classique_honneurs, :continuation_active, opp], false)
  ...
  defp current_partie_trous(trictrac, color) do
    get_in(ensure(trictrac), [:track_classique_honneurs, :current_partie, :trous, color]) || 0
  end
  ...
  defp normalize_current_partie(current_partie) do
    ...
    %CurrentPartie{
      ...
      legal_stop_by: %{
        white: get_in(current_partie, [:legal_stop_by, :white]) || false,
        black: get_in(current_partie, [:legal_stop_by, :black]) || false
      },
      uninterrupted_by: %{
  ...
  defp normalize_track(%Track{} = track, current_partie, _default_track) do
    %Track{
      ...
      continuation_active: normalize_color_map(track.continuation_active, false),
      continuation_resolved_at: normalize_color_map(track.continuation_resolved_at, 0),
  ...
  defp normalize_track(track, current_partie, _default_track) do
    %Track{
      ...
      continuation_active: normalize_color_map(Map.get(track || %{}, :continuation_active), false),
      continuation_resolved_at: normalize_color_map(Map.get(track || %{}, :continuation_resolved_at), 0),
