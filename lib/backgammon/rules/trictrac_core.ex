defmodule Backgammon.Rules.TrictracCore do
  alias Backgammon.Rules.RaceCore
  alias Backgammon.Rules.Trictrac.{AEcrire, Classique, Combine, Plein, Toc}

  @reprise_variants ["trictrac_classique", "trictrac_aecrire", "trictrac_combine"]
  ...
  def pending_options(%{id: "trictrac_aecrire"}),
    do: aecrire_style_options("RuleFrTrictracAEcrire")

  def pending_options(%{id: "trictrac_combine"}),
    do: aecrire_style_options("RuleFrTrictracCombine")
  ...
  defp maybe_finish_classique_match(runtime, %{id: "trictrac_classique"}, color) do
  ...
  defp maybe_finish_combine_match(runtime) do
    cond do
      ...
      winner = get_in(runtime, [:trictrac, :track_classique_honneurs, :winner]) ->
        %{
          runtime
          | match: %{
              runtime.match
  ...
  defp maybe_apply_sortie_releve(runtime, %{id: id} = variant, color)
       when id in ["trictrac_classique", "trictrac_aecrire", "trictrac_combine"] do
  ...
  defp finalize_classique_turn(runtime, previous, variant, color) do
    ...
    runtime =
      update_trictrac(runtime, fn trictrac ->
        trictrac
        |> maybe_record_aecrire_turn(variant, color, deltas)
  ...
  defp build_turn_event_queue(_previous, runtime, %{id: id}, color)
    ...
    reprise =
      if reprise_due?(runtime.trictrac, id, color) and not runtime.match.is_over do
        [
          %{
            "key" => "reprise",
    ...
    continuation =
      if id == @combine_variant and Combine.continuation_due?(runtime.trictrac, color) do
        [
          %{
            "key" => "continuation",
            "prompt" => "Continue the honneurs track?",
            "choices" => ["continuer", "arreter"]
          }
        ]
      else
        []
  ...
  defp apply_turn_decision(runtime, variant, color, "reprise", "s'en aller")
    ...
    settlement_ready? = AEcrire.settlement_ready?(runtime.trictrac, color)
    {trictrac, result} = maybe_resolve_aecrire_reprise(runtime.trictrac, settlement_ready?)
    ...
    runtime =
      ...
      |> update_trictrac(fn _ ->
        updated_trictrac =
          cond do
            variant.id == @combine_variant and settlement_ready? and result.ended_marque ->
              AEcrire.clear_current_coup(trictrac)

            variant.id == @combine_variant ->
              AEcrire.releve_current_coup(trictrac, color)

            settlement_ready? and result.ended_marque ->
              trictrac
  ...
  defp apply_turn_decision(runtime, variant, color, "reprise", "s'en aller")
       when variant.id == "trictrac_classique" do
    fresh = new(variant)
  ...
  defp apply_turn_decision(runtime, variant, color, "continuation", decision)
       when variant.id == @combine_variant do
    runtime
    |> update_trictrac(&Combine.apply_continuation(&1, color, decision))
    |> maybe_finish_combine_match()
    |> shift_pending_event()
    |> maybe_advance_turn_after_queue(variant)
  end
  ...
  defp maybe_record_aecrire_turn(trictrac, %{id: id}, color, deltas)
       when id in @aecrire_variants do
    active? =
      id == "trictrac_aecrire" or
        get_in(trictrac, [:suspension_state, :suspended_track]) != "a_ecrire"

    AEcrire.record_turn(trictrac, color, deltas, active?: active?)
  end

  defp maybe_record_aecrire_turn(trictrac, _variant, _color, _deltas), do: trictrac
  ...
  defp maybe_resolve_aecrire_reprise(trictrac, true), do: AEcrire.resolve_reprise(trictrac)

  defp maybe_resolve_aecrire_reprise(trictrac, false) do
    {trictrac, %{ended_marque: false, refait: false, winner: nil, marque_value: 0}}
  end

  defp reprise_due?(trictrac, "trictrac_classique", _color) do
    get_in(trictrac, [:turn, :can_reprise]) || false
  end

  defp reprise_due?(trictrac, id, color) when id in @aecrire_variants,
    do: AEcrire.reprise_due?(trictrac, color)

  defp reprise_due?(_trictrac, _id, _color), do: false
  ...
  defp aecrire_style_options(rule) do
    %{
      "rule" => rule,
      "options" => [
        %{
