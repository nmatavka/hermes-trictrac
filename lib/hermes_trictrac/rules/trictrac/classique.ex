defmodule HermesTrictrac.Rules.Trictrac.Classique do
  ...
  def begin_turn(trictrac, board, variant, color, dice) do
    ...
    |> put_in(
      [:pending_impuissance_by_type, color],
      VariantRules.impuissance_points(variant, dice, unplayable)
    )
    |> put_in(
  ...
  def finalize_turn(trictrac, board, variant, color, turn_number) do
    ...
    reprise_color =
      Enum.find([:white, :black], fn piece_type ->
        trous_after[piece_type] > turn.trous_before[piece_type]
      end)

    can_reprise = not is_nil(reprise_color)
  ...
  def destination_forbidden_by_jan_interdit?(board, variant, color, destination),
    do: Moves.destination_forbidden_by_jan_interdit?(board, variant, color, destination)
