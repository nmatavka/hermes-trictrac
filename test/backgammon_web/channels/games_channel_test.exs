defmodule BackgammonWeb.GamesChannelTest do
  use BackgammonWeb.ChannelCase, async: true

  alias Backgammon.GameServer
  alias Backgammon.Rules.{Registry, TrictracCore}
  alias BackgammonWeb.UserSocket
  ...
  setup do
    ...
    {:ok, host_reply, host_socket} =
      UserSocket
      |> socket("user:1", %{})
      |> subscribe_and_join(BackgammonWeb.GamesChannel, "games:#{lobby}", %{
        "user" => "nick",
        "variant" => "backgammon",
        "client_id" => "client-1"
      })
  ...
  test "roll broadcasts an updated game", %{host_socket: socket} do
    push(socket, "roll", %{})
    assert_broadcast "update", %{game: %{"dice" => %{"values" => dice}}}
    assert length(dice) in [2, 4]
  end
  ...
  test "channel update serializes trictrac classique reprise decisions" do
    ...
    :sys.replace_state(pid, fn state ->
      ...
      trictrac =
        Backgammon.Rules.Trictrac.Classique.set_turn_event_queue(runtime.trictrac, [pending])
