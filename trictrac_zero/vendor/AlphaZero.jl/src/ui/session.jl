...
function write_atomic(path::String, writer::Function)
  dir = dirname(path)
  isdir(dir) || mkpath(dir)
  tmp_path, io = mktemp(dir)
  try
    writer(io)
    flush(io)
    close(io)
...
function serialize_atomic(path, value)
  write_atomic(path, function(io)
    serialize(io, value)
  end)
end
...
function save_env(env::Env, dir)
  isdir(dir) || mkpath(dir)
  serialize_atomic(joinpath(dir, GSPEC_FILE), env.gspec)
  serialize_atomic(joinpath(dir, PARAMS_FILE), env.params)
  write_json_atomic(joinpath(dir, PARAMS_JSON_FILE), env.params)
  write_json_atomic(joinpath(dir, NET_PARAMS_FILE), Network.hyperparams(env.bestnn))
  serialize_atomic(joinpath(dir, BESTNN_FILE), env.bestnn)
  serialize_atomic(joinpath(dir, CURNN_FILE), env.curnn)
  serialize_atomic(joinpath(dir, MEM_FILE), get_experience(env))
  write_text_atomic(joinpath(dir, ITC_FILE), string(JSON3.write(env.itc)))
end

function replay_buffer_load_error_message(path, err)
  detail = sprint(showerror, err)
  return "Replay buffer at $path is corrupted ($detail). Resuming with an empty experience buffer."
end

function log_replay_buffer_recovery(logger, path, err)
  message = replay_buffer_load_error_message(path, err)
  if isnothing(logger)
    @warn message
  else
    Log.section(logger, 1, "Recovering replay buffer")
    Log.print(logger, Log.RED, message)
  end
end

function load_experience(dir, gspec; logger=nothing)
  path = joinpath(dir, MEM_FILE)
  try
    return deserialize(path), false
  catch err
    log_replay_buffer_recovery(logger, path, err)
    State = GI.state_type(gspec)
    return AlphaZero.TrainingSample{State}[], true
  end
end

function load_env(dir; logger=nothing)
  ...
  experience, recovered = load_experience(dir, gspec; logger)
  itc = open(JSON3.read, joinpath(dir, ITC_FILE), "r") 
  if recovered
    try
      serialize_atomic(joinpath(dir, MEM_FILE), experience)
    catch err
      message = "Failed to rewrite $(joinpath(dir, MEM_FILE)) after recovery ($(sprint(showerror, err)))."
...
function save_report_increment(session, bench, itrep, idir)
  write_json_atomic(joinpath(idir, BENCHMARK_FILE), bench)
  if session.env.itc > 0
    @assert !isnothing(itrep)
    write_json_atomic(joinpath(idir, REPORT_FILE), itrep)
  end
end
