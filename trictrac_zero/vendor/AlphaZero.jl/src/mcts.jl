...
Ntot(b::StateInfo) = sum((s.N for s in b.stats); init=0)
...
function policy(env::Env, game)
  ...
  isempty(actions) && return actions, Float64[]
  ntotal = sum((a.N for a in info.stats); init=0)
  π =
    if iszero(ntotal)
      Float64[a.P for a in info.stats]
    else
      [a.N / ntotal for a in info.stats]
    end
  total_mass = sum(π; init=0.0)
  if iszero(total_mass)
    π = fill(1.0 / length(actions), length(actions))
  else
    π ./= total_mass
  end
  return actions, π
