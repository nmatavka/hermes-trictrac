...
Base.@kwdef struct SelfPlayStragglerPolicy
  timeout_seconds :: Float64
  remaining_games :: Int = 1
end

self_play_straggler_policy(::AbstractGameSpec) = nothing
checkpoint_straggler_policy(::AbstractGameSpec) = nothing

mutable struct SelfPlayStragglerControl
  total_games :: Int
  completed_games :: Int
  last_completion_at :: Float64
  timeout_seconds :: Float64
  remaining_games :: Int
  lock :: ReentrantLock
end

function SelfPlayStragglerControl(total_games::Int, policy::SelfPlayStragglerPolicy)
  return SelfPlayStragglerControl(
    total_games,
    0,
    time(),
    policy.timeout_seconds,
    policy.remaining_games,
    ReentrantLock()
  )
end

function make_self_play_straggler_control(total_games::Int, policy)
...
function remaining_games(control::SelfPlayStragglerControl)
  lock(control.lock)
  try
    return control.total_games - control.completed_games
  finally
    unlock(control.lock)
  end
end

function should_abort_tail_game(control::SelfPlayStragglerControl)
  lock(control.lock)
  try
    remaining = control.total_games - control.completed_games
    idle = time() - control.last_completion_at
    return remaining <= control.remaining_games && idle >= control.timeout_seconds
  finally
    unlock(control.lock)
  end
end

function mark_game_finished!(control::SelfPlayStragglerControl)
  lock(control.lock)
  try
    control.completed_games += 1
    control.last_completion_at = time()
  finally
    unlock(control.lock)
  end
  return nothing
...
function should_abort_game(control::SelfPlayStragglerControl, trace, game)
  return should_abort_tail_game(control)
end
...
function simulate(
  ...
  straggler_control = make_self_play_straggler_control(p.num_games, straggler_policy)
  return Util.mapreduce(1:p.num_games, p.num_workers, vcat, []) do
    ...
    function simulate_game(sim_id)
      ...
      isnothing(straggler_control) || mark_game_finished!(straggler_control)
      game_simulated()
      return isnothing(report) ? Any[] : Any[report]
    end
