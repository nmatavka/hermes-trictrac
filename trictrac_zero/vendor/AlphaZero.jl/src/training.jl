...
function pit_networks(gspec, contender, baseline, params, handler)
  ...
  samples = simulate(
    ...
    straggler_policy=checkpoint_straggler_policy(gspec))
  dropped_games = params.sim.num_games - length(samples)
  if dropped_games > 0
    @warn "$dropped_games checkpoint evaluation game(s) were aborted before completion and omitted from arena scoring."
  end
...
function self_play_step!(env::Env, handler)
  ...
  results, elapsed = @timed simulate_distributed(
    ...
    straggler_policy=self_play_straggler_policy(env.gspec))
  ...
  speed = iszero(elapsed) ? 0.0 : cur_batch_size(env.memory) / elapsed
  edepth = isempty(results) ? 0.0 : mean([x.edepth for x in results])
