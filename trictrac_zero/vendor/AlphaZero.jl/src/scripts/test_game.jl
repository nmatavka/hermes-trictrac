...
function generate_states(gspec, n)
  traces = Any[]
  attempts = 0
  max_attempts = max(n * 4, n)

  while length(traces) < n && attempts < max_attempts
    trace = play_game(gspec, RandomPlayer())
    !isnothing(trace) && push!(traces, trace)
    attempts += 1
  end

  isempty(traces) && error("Could not generate any complete traces for test_game.")
  if length(traces) < n
    @warn "Only generated $(length(traces)) complete traces out of $n requested in test_game; aborted traces were skipped."
  end
