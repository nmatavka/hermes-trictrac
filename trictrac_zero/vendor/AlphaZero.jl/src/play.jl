...
function should_abort_game(watcher, trace, game)
  return false
end

function max_game_length(::Any)
  return nothing
end
...
function play_game(gspec, player; flip_probability=0., tail_watcher=nothing)
  ...
  while true
    ...
    if isempty(GI.available_actions(game))
      return nothing
    end
    if !isnothing(game_length_cap) && length(trace) >= game_length_cap
      return nothing
    end
    if !isnothing(tail_watcher) && should_abort_game(tail_watcher, trace, game)
      return nothing
    end
